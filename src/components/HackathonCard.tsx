...
const HackathonCard: React.FC<Props> = ({ hackathon }) => {
  ...
  const [status, setStatus] = useState<"active" | "upcoming" | "past">(
    "upcoming"
  );
  const [timer, setTimer] = useState<{
    displayTime: string;
    detailedTime: string;
  }>({ displayTime: "", detailedTime: "" });
  ...
  const getStatusChip = () => {
    if (status === "active")
      return <Chip label="Active" color="success" className="chip" />;
    if (status === "upcoming")
      return <Chip label="Upcoming" color="info" className="chip" />;
        ...
        <CardContent className="card-content">
          ...
          <Box className="timer-container">
            ...
            {status === "upcoming" && (
              <>
                <Typography
                  style={{ fontSize: 14, fontWeight: "500", color: "#4F4F4F" }}
                >
                  Starts in:
                </Typography>
                <Typography
