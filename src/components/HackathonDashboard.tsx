...
const HackathonDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (statusFilter.length > 0) {
      filtered = filtered.filter((hack) => {
        const hackStatus =
          new Date(hack.startDate) <= now && new Date(hack.endDate) >= now
            ? "active"
        ...
        return (
          statusFilter.includes("All") || statusFilter.includes(hackStatus)
        );
      });
    }
  ...
  return (
    <Box>
      ...
      <Box className="why-participate-section">
        ...
        >
          Why Participate in{" "}
          <span style={{ color: "#109c27" }}>AI Challenges?</span>
        </Typography>
      ...
      <Box className="explore-challenges-section">
        ...
        <Box
          className="explore-challenges-search"
          display="flex"
          alignItems="center"
          gap={2}
        >
